...
# IdealCalculator
IMPORTANCE_ALPHA: float = 1.6  # importance exponent (1=lineer, 2=karesel)
CASCADE_PASSES: int = 40  # cascade yeniden dağıtım geçişleri
...
class IdealCalculator:
    ...
    def _cascade(self, cat: Category) -> None:
        ...
        alloc: list[float] = [(s / total) * budget / (i.effective_weight or _EPS) for s, i in zip(scores, items)]
    ...
    def _global_rescale(self) -> None:
        ...
        for i in all_items:
            scaled = max(float(i.min_total), min(float(i.max_total), i.ideal_quantity * scale))
...
class SCIPOptimizer:
   ...
   ---
    ...
    def __init__(self, items: list[InventoryItem], usable: float, categories: list[Category]) -> None:
        ...
        N = len(items)
        raw_neg = [W_ITEM_DEV * i.composite_importance * (i.steal_factor + _EPS) for i in items]
        raw_pos = [W_ITEM_DEV * i.composite_importance / (i.steal_factor + _EPS) for i in items]
    ...
    def optimize(self) -> None:
        ...
        # Continuous olanlar sonradan integer'a yuvarlanır; B&B ağacı dramatik küçülür.
        q: dict[int, object] = {}
        ...
        # 3. Kategori give/take (SOFT, linear approx)
        # Give/take penalty: quadratic constraint yerine lineer yaklaşım.
        # give/take değerleri zaten continuous ve [0, T_c] bandında küçük;
        # lineer ceza model yapısını korur, QP constraint sayısını azaltır.
        ...
        # 4. Asimetrik sapma penalty (QP objective via epigraph)-
        # d_neg_i = max(0, ideal_i - q_i)   -> yokluk cezası  (w_neg: imp × sf)
        # d_pos_i = max(0, q_i - ideal_i)   -> varlık cezası  (w_pos: imp / sf)
        #
        # Her QP terimi: epigraph değişkeni t ile  t ≥ w·d²  (quadratic constraint)
        # ANCAK d_neg ve d_pos ayrı linear constraint ile tanımlandığından
        # t_neg ≥ w_n · d_neg²  tek bir QP constraint = 1 QP / item yönü
        # Toplam: 2 × N QP constraint (eskiden 3 × N + floor = 4 × N)
        ...
        # 6. Lineer give/take ceza terimi
        # Quadratic give/take yerine lineer ceza: (1-imp)² × give + (1-imp) × take
        # Yorumsal güç aynı (önemsiz kat kolay verir), QP constraint 0'a düşer.
